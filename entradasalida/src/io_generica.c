...
void copiar_operaciones(INTERFAZ* interfaz){
    ...
    case GENERICA:
        ...
        interfaz->datos->operaciones = malloc(sizeof(cantidad_operaciones));
        for (int i = 0; i < cantidad_operaciones; i++) {
            strcpy(interfaz->datos->operaciones[i], operaciones_gen[i]);
        }
        break;
    case STDIN:
        ...
        interfaz->datos->operaciones = malloc(sizeof(cantidad_operaciones));
        for (int i = 0; i < cantidad_operaciones; i++) {
            strcpy(interfaz->datos->operaciones[i], operaciones_stdin[i]);
        }
        break;
    case STDOUT:
...
void iniciar_interfaz(char* nombre, t_config* config, t_log* logger){
    ...
    //TODO: Al eliminar la interfaz, borrar Interfaz y los strings de Interfaz->datos->nombre, y los de operacion
...
void* gestionar_peticion_kernel(void* args){
	...
	while (1) {
        ...
        case IO_GENERICA:
            ...
            peticion_IO_GEN(nueva_interfaz->nombre, config_generica);
            break;
        case IO_STDIN:
